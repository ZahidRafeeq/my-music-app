...
const Music = () => {
  ...
  const category = location.state?.category || "newrelease";
  ...
  useEffect(() => {
    if (!initialSongs.length && id) {
      const fileMap = {
        newrelease: "/newreleasesongs.json",
        trending: "/trendingsongs.json",
        weeklytop: "/weeklytopsongs.json",
      };
      const filePath = fileMap[category] || "/newreleasesongs.json";
