  ...
  return (
    <Container fluid className="new-release-songs">
      ...
      <div className="d-flex justify-content-between align-items-center mb-3 d-md-none">
        <h6 className="mb-0">New Release <span className="text-pink">Songs</span></h6>
        <span className="view-all-text-only" onClick={handleViewAll} style={{ cursor: 'pointer' }}>
          View All
        </span>
      </div>
      ...
      <Row className="g-4">
        {displayedSongs.map((song, i) => (
          <Col key={song.id} xs={4} sm={3} md={2}>
            <Link
              to={`/music/${song.id}`}
              state={{ songs, currentIndex: i }}
